...
def _validate_question_bank():
    if len(QUESTIONS) != TOTAL_QUESTIONS:
        raise RuntimeError(
            f"Quiz bank mismatch: expected {TOTAL_QUESTIONS}, "
    ...
    for index, question in enumerate(QUESTIONS):
        try:
            ...
            raise RuntimeError(
                f"Invalid quiz question #{index + 1}: {error}"
            ) from error
...
async def _get_question_bank(language: str):
    """
    Two logical banks with identical question IDs:
      QUESTIONS_EN = source bank
      QUESTIONS_FR = French bank generated/cached from the same source.

    French is generated lazily so startup is instant. Each question is
    translated only the first time it is used and then kept in memory.
    """
    if language == "en":
        return QUESTIONS_EN

    return await _get_french_bank()


QUESTIONS_EN = QUESTIONS


async def _get_french_bank():
    """
    French bank with the exact same question IDs as the English bank.

    Entries are translated lazily: only the question currently requested is
    translated, then cached. This prevents /quiz from making 1500 network
    translation requests at once.
    """
    if not hasattr(_get_french_bank, "_cache"):
        _get_french_bank._cache = {}

    return _get_french_bank._cache


async def _get_question(language: str, question_id: int):
    if language == "en":
        return QUESTIONS_EN[question_id]

    cache = await _get_french_bank()

    if question_id in cache:
        return cache[question_id]

    question_text, answers, correct_index = _question_parts(
        QUESTIONS_EN[question_id]
    )

    translated_question = await _french(
        question_text
    )

    translated_answers = [
        await _french(str(answer))
